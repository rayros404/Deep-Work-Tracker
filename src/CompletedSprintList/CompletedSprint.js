...
export default function CompletedSprint(props){
    ...
    return (
        <div className='accordion-item'>
            <h2 className='accordion-header' id={`completed-header${props.item.id}`}>
                <button className='accordion-button collapsed' type='button' data-bs-toggle='collapse' data-bs-target={`#completed-collapse${props.item.id}`} aria-expanded='false' aria-controls={`completed-collapse${props.item.id}`}>
                    {props.item.title}
                </button>
            </h2>
            <div className='accordion-collapse collapse' id={`completed-collapse${props.item.id}`} aria-labelledby={`completed-header${props.item.id}`} data-bs-parent='#completed-sprints-accordion'>
                <div className='accordion-body card'>
                    ...
                    <ul className='list-group list-group-flush'>
                        ...
                        <li className='list-group-item border-0'>
                            <div className='d-flex align-items-center'>
                                ...
                                <span className='mx-3'>{`${getDayName(formatDate(props.item.completeDate))} ${formatDate(props.item.completeDate)}`}</span>
                            </div>
                        </li>
                        <li className='list-group-item border-0'>
                            <div className='d-flex align-items-center'>
                    ...
                    <div className='card-body d-flex justify-content-center'>
                        <button className='btn btn-outline-success btn-lg' onClick={props.handleIncomplete}>
                            Incomplete
                        </button>
                    </div>   
                </div>
